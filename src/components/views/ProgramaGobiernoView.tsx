...
Esta propuesta cumple con los requisitos formales de inscripciones ante la Registraduría Nacional y el CNE, estructurando metas medibles y presupuestadas.`);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* BANNER DE SINCRONIZACIÓN CON GESTIÓN ESTRATÉGICA & PERFIL DE CANDIDATO */}
      <div className="bg-gradient-to-r from-[#031d38] via-[#05284c] to-[#041d33] border border-cyan-500/40 p-4 rounded-2xl flex flex-wrap items-center justify-between gap-4 shadow-xl">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-cyan-500/20 text-cyan-300 rounded-xl border border-cyan-400/40 shadow">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="bg-emerald-500/20 text-emerald-300 text-[10px] font-black px-2.5 py-0.5 rounded-full border border-emerald-400/40 flex items-center gap-1 shadow">
                <CheckCircle2 className="w-3 h-3" /> En Vivo
              </span>
            </div>
            <p className="text-[11px] text-slate-300 mt-0.5 leading-snug">
              Candidato: <strong className="text-amber-300">{programMeta.candidato}</strong> | Partido/Aval: <strong className="text-cyan-300">{programMeta.partidoCoaliccion}</strong> | Territorio: <strong className="text-emerald-300">{programMeta.municipioDepartamento}</strong>
              {lastSyncTime && <span className="text-slate-400 ml-2 font-mono text-[10px]">(Última sincr.: {lastSyncTime})</span>}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handleSyncWithStrategicManagement}
            className="px-3.5 py-2 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 hover:from-cyan-500/30 hover:to-blue-500/30 text-cyan-300 border border-cyan-400/40 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer shadow hover:scale-102"
          >
            <Zap className="w-3.5 h-3.5 text-amber-400" />
            <span>Re-sincronizar Datos</span>
          </button>
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2 max-w-3xl">
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-3 py-1 rounded-full bg-amber-500/20 border border-amber-400/40 text-amber-300 font-black text-[10px] uppercase tracking-wider flex items-center gap-1.5 shadow">
                <BookOpen className="w-3.5 h-3.5 text-amber-400" /> Documento Oficial de Campaña
              </span>
              <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider border ${
                programMeta.estadoRadicacion === 'Radicado Oficial'
                  ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40'
                  : programMeta.estadoRadicacion === 'Aprobado para CNE'
                  ? 'bg-cyan-500/20 text-cyan-300 border-cyan-500/40'
                  : 'bg-amber-500/20 text-amber-300 border-amber-500/40'
              }`}>
                {programMeta.estadoRadicacion}
              </span>
              <span className="text-xs text-slate-400 font-mono">
                📅 Límite CNE: {programMeta.fechaLimiteRadicacion}
              </span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 pt-1 text-xs text-slate-300">
              <span className="flex items-center gap-1 font-semibold text-cyan-300">
                ...
                {programMeta.municipioDepartamento}
              </span>
              <span className="text-slate-500">•</span>
              <span className="flex items-center gap-1 font-semibold text-emerald-300">
                <Award className="w-3.5 h-3.5 text-emerald-400" />
                {programMeta.partidoCoaliccion}
              </span>
            </div>
          </div>
