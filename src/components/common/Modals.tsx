      ...
      {activeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-hidden select-none">
          ...
          <motion.div
            ...
            className="relative z-10 bg-[#040e1e]/92 border border-cyan-500/35 rounded-3xl max-w-lg w-full text-slate-100 p-6 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.85)] backdrop-blur-2xl overflow-hidden max-h-[90vh] flex flex-col pointer-events-auto"
          >
            {/* iOS Tactile Drag Pill Handle */}
            <div className="w-12 h-1.5 bg-slate-500/50 hover:bg-slate-300 rounded-full mx-auto mb-3 cursor-grab active:cursor-grabbing shrink-0 transition-colors" />

            {/* Close Button */}
            <button
              onClick={onClose}
              className="absolute top-4 right-4 text-slate-400 hover:text-white p-1.5 rounded-full hover:bg-cyan-500/20 transition-all cursor-pointer z-20"
            >
              <X className="w-5 h-5" />
            </button>
